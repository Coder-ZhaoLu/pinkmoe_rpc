...
// The SitemetaFunc type is an adapter to allow the use of ordinary
// function as Sitemeta mutator.
type SitemetaFunc func(context.Context, *ent.SitemetaMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f SitemetaFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.SitemetaMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.SitemetaMutation", m)
}
